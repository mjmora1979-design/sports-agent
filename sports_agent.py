...
- nfl_data_py historical stats (NFL only for now)
- Survivor helper
- Excel export
"""
...
# Odds API
# -------------------------------
def get_global_odds(sport="nfl", allow_api: bool = False):
    sport_key = SUPPORTED_SPORTS.get(sport.lower())
    if not sport_key:
        raise RuntimeError(f"Unsupported sport: {sport}")

    files = sorted(glob.glob(f"catalog_{sport_key}_*.json"))
    if files:
        latest = files[-1]
        ts = datetime.strptime(
            latest.replace(f"catalog_{sport_key}_", "").replace(".json", ""),
            "%Y%m%d_%H%M%S"
        )
    ...
    data = r.json()
    json.dump(data, open(f"catalog_{sport_key}_{nowstamp()}.json", "w"), indent=2)
    return data

def get_event_props(event_id: str, sport="nfl", allow_api: bool = False):
    sport_key = SUPPORTED_SPORTS.get(sport.lower())
    if not sport_key:
        raise RuntimeError(f"Unsupported sport: {sport}")
...
def run_model(
    ...
    df_stats = load_weekly_stats() if sport.lower() == "nfl" else pd.DataFrame()
    raw = get_global_odds(sport=sport, allow_api=allow_api)
    ...
    for g in selected:
        ...
        if sport.lower() == "nfl":
            ev = get_event_props(gid, sport=sport, allow_api=allow_api)
            for ed in ev:
                for bm in ed.get("bookmakers", []):
                    for mk in bm.get("markets", []):
                        for outcome in mk.get("outcomes", []):
                            ...
                            model_proj = project_from_history(name, mk.get("key"), df_stats) if not df_stats.empty else None
